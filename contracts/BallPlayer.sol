 ...
 */
contract BallPlayer is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC721("The Ball Player", "BP") {
        MAX_BP_SUPPLY = 10000;
     ...
     * Mint Doges by owner
     */
    function reserveBP(address _to, uint256 _numberOfTokens) external onlyOwner {
    ...
    * Mints tokens
    */
    function mintBP(uint256 numberOfTokens) external payable {
        require(saleIsActive, "Sale must be active to mint");
        require(numberOfTokens <= maxToMint, "Invalid amount to mint per once");
        require(totalSupply().add(numberOfTokens) <= MAX_BP_SUPPLY, "Purchase would exceed max supply");
        require(mintPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
        
        for(uint256 i = 0; i < numberOfTokens; i++) {
            uint256 mintIndex = totalSupply();
            if (totalSupply() < MAX_BP_SUPPLY) {
        ...
        if (startingIndexBlock == 0 && (totalSupply() == MAX_BP_SUPPLY || block.timestamp >= REVEAL_TIMESTAMP)) {
    ...
    function setStartingIndex() external {
        ...
        startingIndex = uint256(blockhash(startingIndexBlock)) % MAX_BP_SUPPLY;
        // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)
        if (block.number.sub(startingIndexBlock) > 255) {
            startingIndex = uint256(blockhash(block.number - 1)) % MAX_BP_SUPPLY;
        }
